...
In your terminal (still in the project folder), type: sqlite3 wtm.db.

This opens the database command line. Now copy and paste these commands one at a time:
```sql
...
This creates the users table where we store account information. Press enter.
```sql
...
This creates the parties table. Press enter.
```sql
...
This creates the posts table for the feed. Press enter.
```sql
...
This creates the comments table. Press enter.
```sql
...
This creates the wishlist table. Press enter.

Now type `.exit` to leave the database command line.

We need to create folders where uploaded photos and flyers will be saved. In your terminal, type:
```
mkdir -p static/uploads/posts static/uploads/parties
```
